...
- 次级文本:`black/[0.34]`。
- 弱文本:`black/[0.22]`。
- 透明窗口 metadata:`black/55`,用于 Project Settings 副标题里的项目路径等必须跨玻璃背景保持可读的辅助信息;不要用浅灰或 `black/[0.22]` 承载长路径。
...
- Bottom sheet 实色面:`--bottom-sheet-surface: #f4f4f1`,用于所有底部 sheet,不要透出底层内容。
...
### Content Density
- Project header:`px-2.5 py-2`,项目名字号 `12px`;不在列表里展示项目路径。项目之间不用分割线,靠留白分组:非首个 project section 追加 `pt-2` 顶部留白(相邻项目约 24px 间距),避免光标题堆叠出表格感。hover 到某个项目时,整组(header + 展开的 worktree)浮现一层中性 hover 底色 `black/[0.038]` + `rounded-[10px]`,给出"各自区域"的边界感;底色是普通中性色,不用项目 accent。留白间隙不属于该 group,悬停间隙不会误触发底色。
- Worktree row:
...
- Header 显示品牌、总 worktree 数和项目数,右侧只放项目定位入口(搜索、总览);不放设置齿轮,避免挤压计数文字。
- Add project 目前是轻量占位入口,不要用说明文字解释功能。
- Footer 放低频全局动作:左侧语言快捷切换 + 版本号,右下角是设置齿轮和 Quit(设置在 Quit 左侧)。

### ProjectSection
- 默认显示前 3 个 worktree,溢出时提供 Show all / Show less。
- hover 项目 header 时,右侧显示 move/settings/new actions。
- 折叠动效使用 `0.22s` 和 `[0.4, 0, 0.2, 1]`。
- 项目拖拽和按钮排序都必须保留动画一致性。

### 项目定位辅助(Type-ahead / Overview)
两者共用 `WorktreePanel.jumpToProject`:平滑滚动到目标 section 顶部并短暂加 `grove-jump-flash`(accent-soft → transparent,`1.1s`,reduce-motion 下不闪)。

- **Type-ahead 跳转**:无 UI 输入框、不筛选。面板激活时直接打字,`WorktreePanel` 全局 keydown 累积 buffer,前缀优先、子串兜底匹配出一组项目并跳到第一个;停顿 1.1s 自动重置。命中多个时 `↑`/`↓` 或滚轮循环切换。顶部中央浮出短暂 chip 回显字母:深色玻璃 `rgba(28,28,32,0.9)` + `shadow-ctx`,`Search` 图标 + mono `12px`,命中用白字 + `#7fb4ff` 图标、未命中置灰划掉,多匹配时右侧加 `当前/总数`。定位主力是键盘,左手定位右手操作。
- 列表不叠加可见的自定义滚动条:原生滚动条隐藏,滚动时也不显示任何滚动条覆盖层(项目名在左侧已够定位)。
- **ProjectOverview**:给鼠标用户的入口。BottomSheet 内容,`bottom-sheet-surface` 面。项目以两列芯片网格铺开,每格 accent `Dot` + 全名(truncate),hover `bg-black/[0.06]`;点击关闭 sheet 并跳转。项目数 ≥2 时 header 显示网格入口。

### WorktreeRow
- 默认显示 branch + subtitle。
- hover 后显示 row actions;对应 action sheet 打开时,row actions 保持可见直到菜单关闭。
- row actions 必须包含一个 `默认打开` icon button,图标使用对应软件 app icon 并随当前默认打开目标切换,tooltip 显示 `默认打开: <目标>`。
- busy 状态隐藏行级操作,显示 spinner + accent 文案。
- commit subtitle 使用弱文本;时间戳进一步降噪。

### ContextMenu
- Worktree actions 由右键或 More 按钮打开,承载方式是底部 sheet,不是指针位置 dropdown。
- Sheet 使用 `motion/react` slide-up 动效,背景只做很轻的 dim,不要遮成 modal。
- 底部 sheet 承载层必须复用 `src/shared/ui/BottomSheet.tsx`;业务组件只提供内部 surface 和内容。
- Sheet 面板使用不透明 `bottom-sheet-surface`、`shadow-ctx`、`MenuItem` 和 `MenuSeparator`,保留原 dropdown menu 的视觉语言但不要透底。
- Worktree actions 保持单层结构;命令类操作用小号标题分组 + 一级菜单项展示,不要恢复 hover 二级 submenu。
- Sheet 打开期间,来源 worktree row 应保持 hover-like 操作状态,减少浮层和行操作之间的视觉跳动。

### Settings Sheets
- Global Settings 和 Project/Workspace Settings 也必须由 `src/shared/ui/BottomSheet.tsx` 承载,不要再替换整个 `PanelShell` 为全屏设置子视图。
- Settings sheet 的背景只做轻 dim,底层项目列表保持可见;内部 surface 使用不透明 `bottom-sheet-surface`、`shadow-panel` 和 window radius。
- Settings 内容组件只负责表单和标题,不要在组件内部再创建 modal/card 外壳。
...
### ProjectSettings
- 项目设置作为 bottom sheet 内容展示,内部复用 settings density token,不是独立 modal card。
...
- 顶部不放返回 Projects 按钮;底部 action row 右对齐,从左到右为 `关闭` 和 `确认`。
- `关闭` 按钮使用 Hero UI `secondary` 视觉,不要自定义成 primary 或裸白底。
...
### GlobalSettings
- 全局设置作为 bottom sheet 内容展示,内部复用 settings density token,不是独立 modal card。
- 入口是 footer 右下角(Quit 左侧)的齿轮按钮,使用共享 `Gear` 图标 + tooltip。
- `默认打开` 使用原生 `<select>` 和 settings field token,选项为 Finder、Zed、Cursor、VS Code、Ghostty 和 Terminal;open target 图标使用本地 app icon 资产,不要用 lucide 通用图标替代。
- 二元偏好使用 Hero UI `Switch` 底座,control/thumb 尺寸必须固定,切换时不应改变行高或挤压文本。
- 顶部不放返回 Projects 按钮;底部提供 `关闭` 按钮且使用 Hero UI `secondary` 视觉,切换项使用 `size="sm"`,标题/描述和 Project Settings 共用 `--settings-*` token。
...
## 无障碍
- Icon-only button 必须有 `title`;共享 `IconButton` 用同一文案生成 `aria-label`、原生 title 和 tooltip。
- 表单控件必须键盘可达。
