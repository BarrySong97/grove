...
- `projects/remove_project.rs`:从 Grove 移除项目,并按全局 remove behavior 可选地对所有 active managed worktree 逐个执行 archive command 和 `git worktree remove`;主 repo 目录永不删除。
...
- `settings/update_app_settings.rs`:保存全局 app settings,当前影响默认打开目标、Ghostty 打开 workspace 的 window/tab 行为、默认 archive 策略和 remove project 行为。
