...
- 新增 Rust command 或 DTO 后必须重新生成本文件;例如 Add project UI 使用 `commands.addProjectFromFolderPicker`,operation UI 使用 `commands.getLatestOperation`/`commands.readOperationLog`,全局 settings DTO 暴露 `language`、`defaultOpenTarget`、`ghosttyOpenMode`、`defaultArchivePolicy` 和 `removeProjectBehavior`。
