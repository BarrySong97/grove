...
## 目录
- `components/`:面板、项目组、worktree 行、右键菜单、打开目标图标、项目/全局设置页和局部编辑器。
...
- `domain/`:纯 Worktree 规则、派生标签、打开目标和命令 catalog。
...
- 全局设置通过 Rust `get_app_settings`/`update_app_settings` 读写 SQLite,当前控制默认打开方式以及 Ghostty 用新 window 还是当前实例 tab 打开 workspace。
...
- 右键或 More 打开 worktree action sheet,支持 open/reveal/copy/run/archive 等入口;open/archive 已接 Rust command,run 仍是占位反馈。
