...
## 职责
`src/modules/worktrees/` 拥有 Grove 的核心面板:项目列表、worktree 行、排序、新建、归档、失败恢复、空状态导入、项目/全局设置和 remove project 入口。

## 目录
- `components/`:面板、项目组、worktree 行、右键菜单、打开目标图标、项目/全局设置页、确认/日志 bottom sheet 和局部编辑器。
- `api/`:前端到 Rust 业务 command 的薄 wrapper,包含项目创建/删除、项目列表、Conductor 导入、创建/retry/archive/open workspace、operation log、项目设置和全局设置 API。
...
- `archiveWorktree` 调用 Rust `archive_workspace`,执行 archive command 并按项目 override 或全局默认策略 hide 或 `git worktree remove`;`ask` 使用 Grove bottom sheet,不使用 `window.confirm`。
...
- 全局设置通过 Rust `get_app_settings`/`update_app_settings` 读写 SQLite,当前控制默认打开方式、Ghostty window/tab、默认 archive 策略和 remove project 行为。
...
- 前端 use-case 中的排序和当前选择只是临时 UI 状态变换,不能当成真实 git/worktree 操作。
...
- Worktree row hover actions 包含 `默认打开` 快捷按钮,按 Global Settings 的 target 调用 `open_workspace`,tooltip 和图标随 Finder/Zed/Cursor/VS Code/Ghostty/Terminal 目标变化;Terminal 快捷按钮固定打开 macOS Terminal。
- 右键或 More 打开 worktree action sheet,支持 open/copy/archive;failed workspace 额外显示 View Log 和 Retry。
...
- `GlobalSettings` 的默认打开方式使用原生 `<select>`,选项来自 `domain/open-targets.ts`;Ghostty 开关使用 Hero UI `Switch` 底座和 `size="sm"`,但 control/thumb 尺寸和颜色收敛到 Grove 紧凑面板。
